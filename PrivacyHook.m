...
static CFPropertyListRef hook_MGCopyAnswer(CFStringRef key, CFDictionaryRef options) {
    ...
    @try {
        ...
        if (CFStringCompare(key, CFSTR("6vURR2UAg"), 0) == 0) {
            NSString *m = getPersistent(@"BdC4.hw", ^{ return versionToMachine(getFakeSystemVersion()); });
            g_inMGHook = NO;
        ...
        if (CFStringCompare(key, CFSTR("J1WLa1FfC0Dowb1A"), 0) == 0) {
            NSString *u = getPersistent(@"BdC4.udid", ^{ return genUUIDStr(); });
            g_inMGHook = NO;
        ...
        if (CFStringCompare(key, CFSTR("wfiZJ9aJbL5A3hMOznwq7A"), 0) == 0) {
            NSString *n = getPersistent(@"BdC4.dn", ^{ return genDeviceName(); });
            g_inMGHook = NO;
...
static CFTypeRef hook_IORegistryEntryCreateCFProperty(io_registry_entry_t entry, CFStringRef key, CFAllocatorRef allocator, uint32_t options) {
    if (key) {
        ...
        if (CFStringCompare(key, CFSTR("IOPlatformUUID"), 0) == 0) {
            NSString *u = getPersistent(@"BdC4.iouuid", ^{ return genUUIDStr(); });
...
// ============================================================
// Persistent fake IDs (BdC4. prefix)
// ============================================================
static NSString *getPersistent(NSString *key, NSString *(^gen)(void)) {
...
static NSString *getFakeID(NSString *name) {
    return getPersistent([NSString stringWithFormat:@"BdC4.ck.%@", name], ^{ return genFakeCookie(name); });
}
...
static BOOL isDeviceKey(NSString *key) {
    ...
    if ([key hasPrefix:@"BdC4"]) return NO;
...
static void initPrivacyHook(void) {
    @autoreleasepool {
        ...
        NSString *fakeMachine = getPersistent(@"BdC4.hw", ^{ return versionToMachine(fakeSV); });
        NSString *fakeDarwin = versionToDarwinRelease(fakeSV);

        NSString *persistedMachine = fakeMachine;
        NSString *memStr = getPersistent(@"BdC4.mem", ^{
        ...
        @try {
            CFPropertyListRef cleared = CFPreferencesCopyAppValue(CFSTR("BdC4.reset"), kCFPreferencesCurrentApplication);
            if (!cleared) {
                ...
                // 1c. 设标记
                CFPreferencesSetAppValue(CFSTR("BdC4.reset"), kCFBooleanTrue, kCFPreferencesCurrentApplication);
                CFPreferencesAppSynchronize(kCFPreferencesCurrentApplication);
            ...
            if (dc) {
                ...
                if (nameM) {
                    IMP imp = imp_implementationWithBlock(^NSString *(id s) {
                        return getPersistent(@"BdC4.dn", ^{ return genDeviceName(); });
                    });
                ...
                if (idfvM) {
                    IMP imp = imp_implementationWithBlock(^NSUUID *(id s) {
                        return [[NSUUID alloc] initWithUUIDString:getPersistent(@"BdC4.iv", ^{ return genUUIDStr(); })];
                    });
        ...
        @try {
            ...
            if (ac) {
                ...
                if (m) {
                    IMP imp = imp_implementationWithBlock(^NSUUID *(id s) {
                        return [[NSUUID alloc] initWithUUIDString:getPersistent(@"BdC4.ai", ^{ return genUUIDStr(); })];
                    });
