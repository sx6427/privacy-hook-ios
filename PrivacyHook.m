//
// PrivacyHook.m — v8: Device spoof + payment compatible
// Cookie: modify on READ only (not on write/store)
// Bundle ID: 3-method hook (Step33 style)
// Device: IDFA/IDFV/name spoof
//
...
static NSMutableURLRequest *modifiedRequest(NSURLRequest *req) {
    ...
    if (cookie.length > 0) {
        ...
        [m setValue:modified forHTTPHeaderField:@"Cookie"];
    }
    // URL params
...
static void initPrivacyHook(void) {
    @autoreleasepool {
        ...
        // ---- 6. NSHTTPCookieStorage — READ ONLY hook ----
        // Only modify cookies when READ (going out with request)
        // Do NOT modify on write/store (server Set-Cookie stored as-is)
        @try {
            ...
            // NOTE: Do NOT hook setCookie: or setCookies:forURL:mainDocumentURL:
            // Server's Set-Cookie is stored as-is, maintaining session integrity
        } @catch (id e) {}

        // ---- 7. Hook NSMutableURLRequest setValue:forHTTPHeaderField: (Cookie only) ----
        @try {
        ...
        // ---- 9. Register NSURLProtocol globally ----
