...
static void initPrivacyHook(void) {
    @autoreleasepool {
        ...
        // 10. JDUpgradeModule router handlers — 延迟到 App 完全启动后
        // 监听 UIApplicationDidBecomeActiveNotification，在 App 显示后再 hook
        @try {
            [[NSNotificationCenter defaultCenter] addObserverForName:@"UIApplicationDidBecomeActiveNotification"
                ...
                usingBlock:^(NSNotification *note) {
                    ...
                    dispatch_once(&onceToken, ^{
                        // 10a. 遍历所有类，找到包含 routerHandle_JDUpgradeModule_ 方法的类
                        @try {
                            ...
                            if (classes) {
                                NSArray *handlerNames = @[
                                    ...
                                    @"routerHandle_AppManagerModule_checkVersion:callback:",
                                ];
                                // 通用的 type encoding: void (id, SEL, id, id)
                                const char *genericType = "v@:@@";
                                for (unsigned int i = 0; i < classCount; i++) {
                                    Class cls = classes[i];
                                    if (!cls) continue;
                                    const char *clsName = class_getName(cls);
                                    if (!clsName) continue;
                                    // 快速检查：类名是否包含 Upgrade 或 AppManager
                                    BOOL nameMatch = NO;
                                    for (const char *p = clsName; *p; p++) {
                                        if (p[0]=='U' && p[1]=='p' && p[2]=='g' && p[3]=='r' && p[4]=='a' && p[5]=='d' && p[6]=='e') { nameMatch = YES; break; }
                                    }
                                    if (!nameMatch) {
                                        for (const char *p = clsName; *p; p++) {
                                            if (p[0]=='A' && p[1]=='p' && p[2]=='p' && p[3]=='M' && p[4]=='a' && p[5]=='n' && p[6]=='a' && p[7]=='g') { nameMatch = YES; break; }
                                        }
                                    }
                                    ...
                                    for (NSString *methodName in handlerNames) {
                                        ...
                                        if (m) {
                                            // 用 method_setImplementation 而不是 class_replaceMethod
                                            // 它不需要传 type encoding，更安全
                        ...
                        @try {
                            ...
                            if (upgradeMgr) {
                                ...
                                if (reqM) {
                                    ...
                                    method_setImplementation(reqM, imp);
                                }
                                ...
                                if (forceM) {
                                    ...
                                    method_setImplementation(forceM, imp);
                                }
