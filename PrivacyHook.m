//
// PrivacyHook.m — v46: Step33 (payment-proven) + Keychain first-only
//
// Based on Step33 which had PAYMENT SUCCESS.
// Changes from Step33:
//   - Keychain clear ONLY on first launch (not every launch)
//   - Otherwise identical to Step33
//
// ONLY: Bundle ID hook (3 methods) + Keychain clear (first only) + IDFA/IDFV + device name
...
static void initPrivacyHook(void) {
    @autoreleasepool {
        ...
        // ---- 1. Clear keychain ONLY on first launch ----
        // Use a persistent flag to check if this is first launch
        @try {
            BOOL firstLaunch = !getPersistent(@"Bdhk.initialized", ^{ return @"YES"; }).boolValue;
            // getPersistent always returns a value (stores "YES" on first call),
            // so we need a different approach: check if the flag exists
            CFStringRef cfKey = (__bridge CFStringRef)@"Bdhk.initialized";
            CFPropertyListRef existing = CFPreferencesCopyAppValue(cfKey, kCFPreferencesCurrentApplication);
            if (!existing) {
                // First launch — clear keychain
                clearKeychain();
                CFPreferencesSetAppValue(cfKey, (__bridge CFStringRef)@"YES", kCFPreferencesCurrentApplication);
                CFPreferencesAppSynchronize(kCFPreferencesCurrentApplication);
            } else {
                CFRelease(existing);
            }
